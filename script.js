...
'how.h2': "働き方",
    ...
    'portfolio.p1.problem': "良い写真の仕上がりは、眺めて憧れるのは簡単でも、実際に再現するのは難しいものです。その技術は、写真がSNSに投稿された瞬間に消えてしまいます。",
    'portfolio.p1.targetUser': "編集をひとつの技術として捉えている、写真編集好きな人たち。SNSで気に入った仕上がりをスクリーンショットしては、真似しようとして諦めてしまうタイプです。",
    'portfolio.p1.competitors': "VSCOやLightroomでもプリセットの保存・共有はできますが、それはあくまで単独のファイルであり、着想元の写真とは切り離されています。気に入った仕上がりを見て、そのレシピをワンタップで手に入れることはできません。アプリの外での現実的な回避策は、仕上がりをスクリーンショットするか、コメントでプリセットを尋ねることです。",
    ...
    'portfolio.p1.solution': "<i>レシピ</i>をアプリの中心的なオブジェクトとして設計しました。フィード、プロフィール、エディターのすべてが、完成した画像を見せるだけでなく、編集内容を読み取ってリミックスできるものである必要がありました。VSCOのロックレイヤー方式を参考にした設計も、意図的な制約のひとつです。他人の編集を生の数値まで完全に分解できないようにし、最大限のリミックス性よりも、元のクリエイターへの帰属を守ることを優先しました。また、よりシンプルなサーバー側レンダリングではなく、Skiaシェーダーで端末上で直接処理する設計を選びました。ソーシャルなループは、ユーザーを待たせた瞬間に崩れてしまうからです。私が下したあるトレードオフは、ポイント制度(投稿やセーブされることへのポイント付与、それを使えるストア)を含む完全なゲーミフィケーション経済を設計しておきながら、それをそのままの形ではリリースしなかったことです。初期のモデリングで、それが「持てる者がさらに持つ」構造を生むとわかったからです。早くから使い始めた人ほど有利さが積み上がり、新規ユーザーが入り込めなくなる。それは発見のためのフィードが本来目指すべきこととは正反対でした。",
    ...
    'portfolio.p1.impact': "ベータユーザーは34人(7月上旬時点)、初週継続率は約21%で、これを伸ばすことを目標にしています :)",
...
'portfolio.p2.title': "レビュー感情分析プラットフォーム - インタラクティブマップ",
    ...
    'portfolio.p2.competitors': "既存のレビューダッシュボード(Google自体の平均評価や、TripAdvisorのような集計サービス)は静的なスコアしか示さず、時間や地域による変化はわかりません。もちろん手作業の代替策として、レビューを一件ずつ読んで感情を手で集計するという方法もあります。正確ではありますが、何千件ものレビューからパターンを見つけるにはあまりに遅すぎます。",
    'portfolio.p2.insight': "最初のバージョンは連続的なヒートマップで表示していて、一見説得力があるように見えましたが、実際はそうではありませんでした。ヒートサーフェスは地理空間上の補間を前提としますが、実際のデータは離散的で、サンプリングの密度も場所によって偏ったレビュー点の集まりです。中心部の密集したブロックと、まばらな住宅地では統計的な重みがまったく異なります。そのため最初のバージョンでは、グラデーションが利用可能なデータの量を、あたかも「ここの感情がどれだけ強いか」であるかのように表示していたのです。解決策は個別のマーカーに切り替えることでした。密度と強度がひとつの誤解を招く色に混同されないよう、それぞれをサイズと色で分けて表現しました。",
    'portfolio.p2.solution': "Googleレビューをスクレイピングし、星評価だけでなくテキスト自体にRoBERTaによる感情分析をかけ、その結果を年スライダー(時間の経過による変化がまず最初に目に入るように)と平均期間スライダー付きのLeafletマップ上に表示しました。より難しかった設計判断は、前述の通り、データを滑らかにしたい誘惑に抵抗することでした。あえて作らなかったもの:複数都市への対応や一般公開版です。このプロジェクトはひとつの地区、ひとつの研究課題に意図的に絞り込んでおり、一般化すればこのプロジェクトの本質だった年単位の精度を犠牲にすることになったはずです。",
...
'portfolio.p3.title': "音声操作型VR建築スタジオ",
    ...
    'portfolio.p3.problem': "3Dの建物コンセプトデザインを試行錯誤するのは時間がかかります。主なボトルネックは、アイデアを素早く可視化可能な3Dの形にする際の摩擦です。特に3Dソフトやゲーム/VRの操作に慣れていない建築家にとっては、それが大きな課題でした。",
    ...
    'portfolio.p3.targetUser': "技術系の専門家(建築家や、初期段階のコンセプトに取り組む建築学生)と、デザイン志向のアマチュア全般。ゲーム機や3D操作に慣れている人だけを想定してはいません。",
    'portfolio.p3.competitors': "RhinoやSketchUpのような従来型ツールは精度が高い一方、依然としてかなり正確なデータ入力を求められるため、初期段階のコンセプト作業を素早く試行錯誤するには向いていません。手作業での回避策は今も発泡スチロールや厚紙のマッシングモデルです。Gravity Sketchのような既存のVRデザインツールは、空間内で3D作業をする部分は解決していますが、依然としてハンドコントローラーに依存しており、ゲーマーではない建築家にとって取り除きたかった障壁をそのまま再導入してしまいます。",
    'portfolio.p3.insight': "最初のバージョンは、壁の種類を選ぶ、寸法を調整する、素材を変えるといった一般的な操作を、典型的なVRアプリのパターン通り、まだメニューに頼っていました。しかしこれはVRという媒体そのものと相性が悪いものでした。建築家たちは空間の中でリアルタイムに考え、試行錯誤しようとしているのに、メニューに潜るたびにその流れが断ち切られてしまっていたのです。私たちが気づいたのは、問題そのものが「操作にかかるコスト」にあるということでした。どのメニュー項目がわかりにくいかではなく。Geminiを介した音声操作によって、階層をたどる代わりに、変更したい内容を普通の言葉で説明できるようになりました。",
    'portfolio.p3.solution': "ユーザーが欲しいものを言葉で説明すると、Gemini APIがその自由な自然言語を構造化された3Dアクションへと変換するVRスタジオを構築しました。システムが何を理解したかをユーザーが確認してから確定できるよう、キーワード要約も画面に表示しています。これは、正確な入力ではなく曖昧さと意図のためにデザインすることを意味していました。実際の作業の多くは、人が実際に使う言い回しに対してシステムを柔軟にすることに費やされ、それは9人のユーザーと7回のデザイン改善を重ねることでしか見つけられませんでした。あえて作らなかったものも同じくらい重要です。複数人での共同作業、そして本当の意味での自由な音声入力です。発話はGeminiを通して処理しましたが、それでも意図の範囲はある程度限定し、任意のコマンドをすべて受け付けるようにはしませんでした。本当に重要なコマンドの信頼性を優先して、コマンドの幅を犠牲にしたということです。",
    'portfolio.p3.feedback': "もっとも明確な反応は、行動から得られたものでした。当たり前だと思っていたコマンドで誰かがつまずく様子を見ることは、後から聞くどんな言葉よりも役に立ちました。「当たり前」のコマンドが、最初から本当に当たり前だったことはほとんどありませんでした。",
...
'portfolio.p4.title': "視覚障がい者のための画像処理とメカトロニクス",
    ...
    'portfolio.p4.targetUser': "試合を生で観戦する視覚障がいのある観客。プレーが実際に起きている最中にそれを追えるだけの速さで、位置情報を必要としています。",
    ...
    'portfolio.p4.insight': "最初のバージョンでは、より重く高解像度な検出モデルを使い、精度を優先して遅延は二の次として扱っていました。実際のエッジ/Arduino側のパイプラインで動かした途端、それは破綻しました。2秒の遅れがあれば、システム全体が誤解を招くものになるには十分です。すでに終わったプレーを感じることになってしまうからです。気づきは、精度は意味を持つタイミングで届いて初めて価値を持つという点にありました。だからこそ、高い精度をフレームレートと遅延のために手放すことにし、低解像度入力のYOLOv9モデルへと向かったのです。",
    'portfolio.p4.solution': "遅延を最も重要な制約とし、その代わりとして許容できる誤差を受け入れました:推論時間180ミリ秒、位置精度は約6メートルです。それを12cmの感覚ボード上で指に感じられる触覚バンドへと変換しています。この±6メートルの誤差は、物理システム自体が小さいこともあり、「ボールは左サイド、ペナルティエリア付近」と伝えるには十分な精度です。ハードウェアについても同じ考え方です。「明らかに良さそう」な角度を推測するのではなく、実際のカバー範囲と精度の最適なバランスを見つけるために60通りのカメラ配置を検証しました。作らなかったもの:全選手のトラッキングシステムです。それを作れば、視覚障がいのある観客が観客の歓声や解説からすでにある程度得ている情報のために、遅延と複雑さを増やすことになったはずです。",
    'portfolio.p4.impact': "フランスの理工系学生向け研究コンテストTIPEで全国上位3%にランクインし、20/20の評価を得ました。選手や観客など、スポーツを取り巻くさまざまな立場の人たちを想像したことが、このプロジェクトの出発点でした。「取り残されているのは誰か」から始めること。それが今も変わらない、私の課題の選び方です。",
  ...
  de: {
    ...
    'portfolio.p1.problem': "Ein guter Foto-Look lässt sich leicht bewundern, aber meist nur schwer reproduzieren. Die Technik dahinter verschwindet in dem Moment, in dem das Foto auf einer Social-Media-Plattform gepostet wird.",
    'portfolio.p1.targetUser': "Hobby-Fotobearbeiter:innen, die Bearbeitung bereits als Handwerk begreifen: Sie machen Screenshots von Looks, die sie in sozialen Medien lieben, geben aber auf, wenn sie versuchen, sie nachzubilden.",
    'portfolio.p1.competitors': "VSCO und Lightroom lassen Nutzer:innen Presets speichern und teilen, aber als eigenständige Dateien, losgelöst von dem Foto, das sie inspiriert hat. Man kann keinen Look sehen, den man liebt, und mit einem Tap dessen genaues Rezept bekommen. Außerhalb von Apps besteht der eigentliche Workaround heute darin, einen Look als Screenshot zu speichern oder in den Kommentaren nach dem Preset zu fragen.",
    ...
    'portfolio.p1.impact': "34 Beta-Nutzer:innen (Stand Anfang Juli), mit einer Woche-1-Retention von etwa 21% - eine Kennzahl, auf die ich hinarbeite :)",
    ...
    'portfolio.p2.targetUser': "Die Forschenden des Sozialwissenschafts-Teams der Hochschule, die Muster in der öffentlichen Meinung über die Zeit erkennen mussten.",
    'portfolio.p2.competitors': "Fertige Bewertungs-Dashboards (Googles eigener Bewertungsdurchschnitt, TripAdvisor-artige Aggregatoren) zeigen nur einen statischen Wert, nicht wie er sich über Zeit oder Raum verändert. Eine manuelle Alternative wäre natürlich gewesen, Bewertungen einzeln zu lesen und das Sentiment von Hand zu zählen. Genau, aber viel zu langsam, um ein Muster über Tausende Bewertungen hinweg zu erkennen.",
    'portfolio.p2.insight': "Die erste Version zeigte eine durchgehende Heatmap, die autoritativ wirkte, es aber nicht war. Eine Wärmefläche unterstellt Interpolation über den Raum hinweg, aber die zugrundeliegenden Daten sind tatsächlich diskrete, ungleichmäßig verteilte Bewertungspunkte. Ein dichter Innenstadtblock und ein dünn besiedeltes Wohnviertel haben nicht dasselbe statistische Gewicht. In der ersten Version codierte der Farbverlauf also die Menge der verfügbaren Daten, so als wäre es „wie stark ist das Sentiment hier“. Die Lösung war der Wechsel zu einzelnen Markern, deren Größe und Farbe Dichte und Intensität getrennt hielten, statt sie in einer einzigen irreführenden Farbe zu vermischen.",
    'portfolio.p2.solution': "Wir haben Google-Bewertungen gescrapt, eine RoBERTa-Sentiment-Analyse auf den eigentlichen Text angewendet (nicht nur auf Sternebewertungen), und das Ergebnis auf einer Leaflet-Karte mit einem Jahres-Schieberegler (damit Veränderung über die Zeit das Erste ist, was man sieht) und einem Schieberegler für den Mittelungszeitraum dargestellt. Die schwierigere Design-Entscheidung war, wie bereits erwähnt, dem Drang zu widerstehen, die Daten zu glätten. Was wir bewusst nicht gebaut haben: Unterstützung für mehrere Städte oder eine öffentliche Version. Dieses Projekt war bewusst eng auf eine Forschungsfrage für ein Viertel zugeschnitten, und es zu verallgemeinern hätte bedeutet, die Jahr-für-Jahr-Präzision zu opfern, die den eigentlichen Sinn der Sache ausmachte.",
    ...
    'portfolio.p3.problem': "An einem 3D-Gebäudekonzept zu iterieren ist langsam. Der Hauptengpass ist die Reibung, Ideen schnell in eine darstellbare 3D-Form zu bringen - besonders für Architekt:innen, die mit 3D-Software und VR-/Gaming-Steuerungen wenig vertraut sind.",
    ...
    'portfolio.p3.targetUser': "Technische Profis (Architekt:innen, Architekturstudierende, die an frühen Konzepten arbeiten) sowie designorientierte Amateur:innen im Allgemeinen - nicht zwingend Menschen, die mit spielähnlichen oder 3D-Steuerungen vertraut sind.",
    'portfolio.p3.competitors': "Klassische Werkzeuge (Rhino, SketchUp) sind präzise, aber langsam zu iterieren für frühe Konzeptarbeit, da sie weiterhin recht genaue Eingaben verlangen. Der manuelle Workaround ist nach wie vor ein physisches Massenmodell aus Schaumstoff oder Karton. Bestehende VR-Design-Tools wie Gravity Sketch lösen den Teil „in 3D im Raum arbeiten“, setzen aber weiterhin auf Handcontroller, was genau die Hürde wieder einführt, die wir für nicht-gamende Architekt:innen beseitigen wollten.",
    'portfolio.p3.insight': "Die erste Version setzte für gängige Aktionen (etwa einen Wandtyp wählen, eine Abmessung anpassen, ein Material austauschen) noch auf Menüs, das übliche VR-App-Muster. Das kämpfte gegen das Medium an: Architekt:innen versuchten, in Echtzeit räumlich zu denken und zu iterieren, und jeder Ausflug ins Menü unterbrach diesen Fluss. Uns wurde klar, dass die Interaktionskosten selbst das Problem waren, nicht welcher Menüpunkt falsch beschriftet war. Sprache, geleitet über Gemini, ließ Menschen eine Änderung in normalen Worten beschreiben, statt sich durch eine Hierarchie zu klicken.",
    'portfolio.p3.solution': "Wir haben ein VR-Studio gebaut, in dem die Nutzer:innen beschreiben, was sie wollen, und die Gemini API diese lockere natürliche Sprache in strukturierte 3D-Aktionen übersetzt - mit einer Stichwort-Zusammenfassung, die zeigt, was das System verstanden hat, bevor man bestätigt. Das bedeutete, für Mehrdeutigkeit und Absicht zu gestalten statt für präzise Eingaben. Der Großteil der eigentlichen Arbeit bestand darin, das System tolerant dafür zu machen, wie Menschen Dinge wirklich formulieren, was wir nur durch 7 Design-Iterationen mit 9 Nutzer:innen herausfanden. Was wir bewusst nicht gebaut haben, war genauso wichtig: Mehrbenutzer-Zusammenarbeit, und wirklich freie Spracheingabe - wir haben Sprache über Gemini geleitet, aber den Intentionsraum trotzdem eingegrenzt, statt beliebige Befehle zu akzeptieren, und damit Bandbreite gegen Zuverlässigkeit bei den Befehlen getauscht, die wirklich zählten.",
    'portfolio.p3.feedback': "Das eindeutigste Feedback war verhaltensbasiert: zuzusehen, wie jemand an einem Befehl scheitert, den wir für offensichtlich hielten, war nützlicher als alles, was danach gesagt wurde. Der „offensichtliche“ Befehl war es beim ersten Versuch selten wirklich.",
    ...
    'portfolio.p4.targetUser': "Blinde und sehbehinderte Fußballzuschauer:innen bei einem Live-Spiel, die Positionsinformationen schnell genug brauchen, um eine Spielszene mitzuverfolgen, während sie passiert.",
    ...
    'portfolio.p4.insight': "Mein erster Durchlauf nutzte ein schwereres, höher auflösendes Erkennungsmodell und jagte zunächst der Genauigkeit hinterher, Latenz war zweitrangig. Das brach zusammen, sobald es auf dem tatsächlichen Edge-/Arduino-Pfad laufen musste. Eine Verzögerung von zwei Sekunden reicht aus, um das gesamte System irreführend zu machen, weil man dann eine bereits vergangene Aktion spüren würde. Der Durchbruch war die Erkenntnis, dass Genauigkeit nur zählt, wenn sie rechtzeitig ankommt, um etwas zu bedeuten. Deshalb habe ich hohe Präzision gegen Bildrate und Latenz eingetauscht, was mich zu einem YOLOv9-Modell mit niedrig aufgelöstem Input geführt hat.",
    'portfolio.p4.solution': "Ich habe Latenz zur wichtigsten Einschränkung gemacht und im Gegenzug eine vertretbare Fehlermarge akzeptiert: 180 ms Inferenzzeit und etwa 6 Meter Lokalisierungsfehler, umgewandelt in eine haptische Bande, die die Nutzer:innen unter den Fingern auf einem 12-cm-Sensorboard spüren. Dieser Fehler von +/-6 m ist präzise genug, um „Ball am linken Flügel, nahe dem Strafraum“ zu vermitteln (da das physische System eher klein ist). Dieselbe Logik gilt für die Hardware: Ich habe 60 Kamerapositionen getestet, um den tatsächlichen Sweet Spot zwischen Abdeckung und Genauigkeit zu finden, statt einen „offensichtlich guten“ Winkel zu erraten. Was ich nicht gebaut habe: ein System zur Verfolgung aller Spieler:innen. Das hätte Latenz und Komplexität für Informationen hinzugefügt, die sich blinde Zuschauer:innen teilweise bereits aus Publikumslärm und Kommentar erschließen können.",
    'portfolio.p4.impact': "Landesweit unter den besten 3% beim TIPE-Forschungswettbewerb für Ingenieurstudierende in Frankreich platziert, mit 20/20 bewertet. An all die verschiedenen Menschen rund um den Sport zu denken - Spieler:innen, Zuschauer:innen - hat mich zu diesem Projekt geführt. Von „Wer wird ausgeschlossen?“ auszugehen, ist bis heute meine Art, Probleme auszuwählen.",
