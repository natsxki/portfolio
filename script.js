...
const translations = {
  fr: {
    ...
    'portfolio.p1.problem': "Un bon rendu photo s'admire facilement mais se reproduit très difficilement : la technique reste enfermée dans l'appli, ou dans la tête de la personne qui l'a retouché, et elle disparaît dès que la photo est publiée.",
    'portfolio.p1.targetUser': "Les passionnés de retouche photo amateur, qui traitent déjà la retouche comme un savoir-faire - le genre de personne qui capture un rendu qu'elle aime, puis abandonne en essayant de le reproduire.",
    'portfolio.p1.competitors': "VSCO et Lightroom permettent de sauvegarder et partager des préréglages, mais sous forme de fichiers isolés, déconnectés de la photo qui les a inspirés - impossible de voir un rendu qu'on aime et d'obtenir sa recette exacte en un geste. En dehors des applis, la solution de contournement actuelle consiste à capturer un rendu et demander « c'est quel filtre ? » en commentaire, puis à deviner.",
    'portfolio.p1.insight': "La plupart des applis photo partagent le résultat : l'image finie. Mais ce qui a réellement de la valeur, ce qui est transférable, ce n'est pas le résultat, c'est la transformation. Mon premier réflexe penchait vers une place de marché de préréglages - des rendus statiques qu'on applique, pas si différents d'un pack de filtres. Ça n'a pas fonctionné dès les premières maquettes : un filtre est statique, il ne s'adapte pas à la photo. Le déclic est venu en voyant la vraie forme du problème : quelqu'un demande « comment t'as eu ce rendu ? », et la réponse honnête est un paragraphe de réglages que personne ne va reproduire à la main. Cet écart, entre admirer un rendu et réussir à le reproduire, c'est le produit.",
    'portfolio.p1.solution': "J'ai fait de la recette l'objet central de l'appli : le fil, le profil et l'éditeur devaient tous rendre les retouches lisibles et remixables, pas seulement afficher des images finies. Le système de calques verrouillés (inspiré de VSCO) était aussi une contrainte délibérée : j'ai choisi de ne pas laisser les utilisateurs déconstruire entièrement la retouche de quelqu'un d'autre jusqu'aux valeurs brutes, en sacrifiant la remixabilité maximale pour préserver l'attribution du créateur original. Et j'ai choisi de faire tourner les retouches sur l'appareil avec des shaders Skia plutôt qu'un rendu côté serveur plus simple, car une boucle sociale meurt dès qu'ajuster donne une impression d'attente. Le compromis dont je peux parler le plus honnêtement : j'ai conçu une économie de gamification complète - des points pour publier, des points quand on est sauvegardé, une boutique pour les dépenser - puis j'ai choisi de ne pas la déployer telle quelle. Mes premières modélisations montraient qu'elle créerait une dynamique de « les riches s'enrichissent » : les premiers utilisateurs à sauvegarder accumulent un avantage, et les nouveaux arrivants ne peuvent pas percer - l'inverse de ce qu'un fil de découverte devrait faire. J'ai aussi mis de côté la vidéo et un fil algorithmique dans un premier temps, car les deux auraient compliqué le problème de « la recette comme objet lisible » avant même d'avoir prouvé que la boucle centrale fonctionnait.",
    'portfolio.p1.feedback': "Toujours en bêta fermée, donc le signal le plus clair pour l'instant vient de l'usage, pas d'un sondage à grande échelle. La rétention est le chiffre que je surveille le plus, et la gamification est l'expérimentation en cours pour l'améliorer. Si vous lisez ceci et voulez devenir bêta-testeur, faites-moi signe !",
    'portfolio.p1.impact': "Environ 34 bêta-testeurs pour l'instant, avec une rétention à J+7 (semaine 1) d'environ 21%, la métrique que je considère comme la plus importante.",
    ...
    'portfolio.p2.targetUser': "Les chercheurs de l'équipe de sciences sociales de l'école, qui avaient besoin de voir des motifs dans l'opinion publique au fil du temps, pas seulement de lire les avis un par un.",
    'portfolio.p2.competitors': "Les tableaux de bord d'avis classiques (la note moyenne Google elle-même, les agrégateurs façon TripAdvisor) n'affichent qu'un score statique, pas son évolution dans le temps ou l'espace. La solution de contournement des chercheurs avant ce projet était de lire les avis un par un et de compter le sentiment à la main - précis, mais bien trop lent pour repérer un motif parmi des milliers d'avis.",
    'portfolio.p2.insight': "La première version affichait une carte de chaleur continue, ce qui semblait faire autorité mais ne l'était pas. Une surface de chaleur suppose une interpolation à travers l'espace, alors que les données sous-jacentes sont en réalité des points d'avis discrets et inégalement répartis - un pâté de maisons dense au centre-ville et un quartier résidentiel épars n'ont pas le même poids statistique, donc le dégradé encodait silencieusement « combien de données existent ici » comme s'il s'agissait de « à quel point le sentiment est fort ici ». La solution a été de passer à des marqueurs individuels, dimensionnés et colorés pour que densité et intensité cessent d'être confondues en une seule couleur trompeuse.",
    'portfolio.p2.solution': "Nous avons scrapé des avis Google, fait tourner une analyse de sentiment RoBERTa sur le texte lui-même (pas seulement les notes en étoiles, plus grossières), et affiché le résultat sur une carte Leaflet avec un curseur d'année, pour que le changement dans le temps soit la première chose qu'on voit. La décision la plus difficile a été de résister à l'envie de lisser les données : au lieu d'interpoler les couleurs entre les points, je les ai représentées individuellement, car le lissage est plus joli mais ment discrètement avec aussi peu de points par lieu. Ce que nous avons choisi de ne pas construire : un support multi-villes ou une version publique - ce projet était volontairement cadré sur une seule question de recherche pour un seul quartier, et le généraliser aurait signifié sacrifier la précision d'une année sur l'autre qui était tout l'intérêt du projet.",
    ...
    'portfolio.p3.problem': "Itérer sur un projet 3D de bâtiment est lent, et le vrai frein n'est pas les idées : c'est la friction pour les transformer rapidement en une forme 3D exploitable, en particulier pour des architectes qui ne sont pas gamers.",
    ...
    'portfolio.p3.targetUser': "Les architectes et étudiants en architecture qui prototypent des concepts de bâtiment en phase amont - professionnels techniques comme amateurs orientés design, mais pas des personnes à l'aise avec des manettes de jeu.",
    'portfolio.p3.competitors': "Les outils traditionnels (Rhino, SketchUp) sont précis mais lents à itérer pour un travail de concept amont, et la solution de contournement manuelle reste la maquette physique en mousse ou carton. Les outils VR existants comme Gravity Sketch résolvent la partie « travailler en 3D dans l'espace », mais reposent toujours sur des manettes - ce qui réintroduit exactement l'obstacle qu'on cherchait à supprimer pour des architectes non-gamers.",
    'portfolio.p3.insight': "La première version reposait encore sur des menus pour les actions courantes - choisir un type de mur, ajuster une dimension, changer un matériau - le schéma classique des applis VR. Ça allait à contre-courant du medium : les architectes essayaient de penser et d'itérer dans l'espace en temps réel, et chaque plongée dans un menu cassait ce flux, les ramenant à une interaction 2D à l'intérieur d'un outil censé être en 3D. Le déclic n'était pas un bug qu'on a trouvé : c'était de réaliser que le coût d'interaction lui-même était le problème, pas quel élément de menu était mal étiqueté. La voix, passée par Gemini, a permis de décrire un changement en langage courant plutôt que de fouiller dans une hiérarchie.",
    'portfolio.p3.solution': "Nous avons construit un studio VR où l'on décrit ce qu'on veut, et l'API Gemini transforme ce langage naturel en actions 3D structurées, avec un résumé en mots-clés montré à l'utilisateur pour qu'il voie ce que le système a compris avant de valider. Cela impliquait de concevoir pour l'ambiguïté et l'intention plutôt que pour une saisie précise - l'essentiel du travail a consisté à rendre le système tolérant à la façon dont les gens formulent réellement les choses, ce qu'on n'a trouvé qu'en menant 7 itérations de design avec 9 utilisateurs avant de passer au casque. Ce que nous avons choisi de ne pas construire comptait tout autant : la collaboration multi-utilisateurs, et une voix vraiment libre - on a fait passer la parole par Gemini, mais en cadrant quand même l'espace des intentions plutôt que d'accepter des commandes arbitraires, échangeant l'étendue des commandes contre la fiabilité de celles qui comptaient vraiment. Pas de physique, de persistance ni de pipeline d'export non plus : l'outil était cadré pour l'itération rapide en phase amont, pas pour un pipeline de production complet.",
    'portfolio.p3.feedback': "Le retour le plus clair était comportemental, pas verbal : voir quelqu'un buter sur une commande qu'on pensait évidente était plus utile que tout ce qu'il aurait pu dire après coup. C'est ce qui a guidé la plupart des 7 itérations - la commande « évidente » l'était rarement dès le premier essai.",
    ...
    'portfolio.p4.problem': "Les spectateurs malvoyants profitent du son et de l'ambiance du stade, mais perdent la seule information qui explique vraiment le jeu : où se trouve le ballon, à l'instant présent.",
    ...
    'portfolio.p4.targetUser': "Les spectateurs aveugles et malvoyants assistant à un match en direct, qui ont besoin d'une information de position assez rapide pour suivre une action pendant qu'elle se déroule, pas après coup.",
    'portfolio.p4.competitors': "Le commentaire audio existe déjà dans la plupart des stades, mais c'est une narration humaine en phrases, pas une donnée de position continue - il y a un décalage inhérent, et ça ne peut pas transmettre où se trouve exactement le ballon sur le terrain, seconde par seconde. La solution de contournement manuelle la plus courante aujourd'hui est un accompagnant voyant assis à côté du spectateur, qui commente discrètement la position du ballon en temps réel.",
    'portfolio.p4.insight': "Ma première version utilisait un modèle de détection plus lourd et à plus haute résolution, en privilégiant la précision et en traitant la latence comme secondaire. Ça s'est effondré une fois confronté au vrai pipeline embarqué/Arduino : une position spatialement précise du type « le ballon est à votre gauche » qui arrive deux secondes trop tard n'est pas juste inutile, elle est activement trompeuse, puisqu'on ressentirait une action déjà terminée. Le déclic a été de réaliser que la précision ne compte que si elle arrive à temps pour signifier quelque chose - j'ai donc échangé la précision et la résolution du modèle contre la fréquence d'images et la latence, ce qui m'a orientée vers YOLOv9 sur un pipeline plus léger plutôt qu'un détecteur plus lourd.",
    'portfolio.p4.solution': "J'ai fait de la latence la contrainte principale, en acceptant en échange une marge d'erreur tolérable : 180 ms d'inférence et environ 6 mètres de précision de localisation, convertis en une bande haptique que l'utilisateur ressent sous ses doigts sur un boîtier sensoriel de 12 cm. Cette erreur de ~6 m est assez précise pour transmettre « le ballon est sur l'aile gauche, près de la surface » sans avoir besoin de coordonnées parfaites. La même logique a guidé le matériel : j'ai testé 60 emplacements de caméra pour trouver le vrai équilibre couverture/précision, plutôt que de deviner un angle « évidemment bon ». Ce que j'ai choisi de ne pas construire : un système de suivi de tous les joueurs - cela aurait ajouté de la latence et de la complexité pour une information qu'un spectateur aveugle perçoit déjà en partie grâce au bruit de la foule et au commentaire ; la position du ballon était le seul signal qui manquait vraiment.",
    'portfolio.p4.impact': "Classée dans le top 3% national au TIPE (concours de recherche pour élèves ingénieurs en France), notée 20/20. Penser à tous les profils qui existent autour du sport - joueurs, spectateurs - m'a menée à ce projet. Partir de « qui est laissé de côté ? » plutôt que de « qu'est-ce qui est techniquement impressionnant ? » reste ma manière préférée de choisir mes problèmes !",
...
'how.h2': "働き方",
    ...
    'portfolio.p1.problem': "良い写真の仕上がりは、眺めて憧れるのは簡単でも再現するのはほぼ不可能です。その技術はアプリの中か編集者の頭の中にとどまり、投稿された瞬間に消えてしまいます。",
    'portfolio.p1.targetUser': "編集をひとつの技術として捉えている、写真編集好きな人たち - 好きな仕上がりをスクリーンショットしては、真似しようとして諦めてしまうタイプの人です。",
    'portfolio.p1.competitors': "VSCOやLightroomでもプリセットの保存・共有はできますが、それはあくまで単独のファイルであり、着想元の写真とは切り離されています。気に入った仕上がりを見て、そのレシピをワンタップで手に入れることはできません。アプリの外での現実的な回避策は、仕上がりをスクリーンショットして「これ何のフィルター?」とコメントで聞き、あとは目分量で真似することです。",
    'portfolio.p1.insight': "多くの写真編集アプリは「結果」、つまり完成した画像を共有します。しかし本当に価値があり、他の人にも伝えられるのは結果そのものではなく「変換の過程」です。最初に思いついたのは、フィルターパックとそう変わらない、静的なルックを適用するプリセットのマーケットプレイスでした。しかし初期のスケッチの段階でうまくいかないとわかりました。フィルターは静的で、実際の写真に適応しないからです。本当の課題の形が見えたのは、誰かが「どうやってその仕上がりにしたの?」と聞き、正直な答えが誰も手作業では再現できないような設定の羅列になる、そんな瞬間を思い描いたときでした。ある仕上がりに憧れることと、実際にそれを再現できることの間にあるこのギャップこそが、プロダクトなのです。",
    'portfolio.p1.solution': "レシピをアプリの中心的なオブジェクトとして設計しました。フィード、プロフィール、エディターのすべてが、完成した画像を見せるだけでなく、編集内容を読み取ってリミックスできるものである必要がありました。VSCOのロックレイヤー方式を参考にした設計も、意図的な制約のひとつです。他人の編集を生の数値まで完全に分解できないようにし、最大限のリミックス性よりも、元のクリエイターへの帰属を守ることを優先しました。また、よりシンプルなサーバー側レンダリングではなく、Skiaシェーダーで端末上で直接処理する設計を選びました。ソーシャルなループは、調整に待たされていると感じた瞬間に崩れてしまうからです。もっとも正直に語れるトレードオフは、ポイント制度(投稿やセーブされることへのポイント付与、それを使えるストア)を含む完全なゲーミフィケーション経済を設計しておきながら、それをそのままの形ではリリースしないと判断したことです。初期のモデリングで、それが「持てる者がさらに持つ」構造を生むとわかったからです。早くから使い始めた人ほど有利さが積み上がり、新規ユーザーが入り込めなくなる。それは発見のためのフィードが本来目指すべきこととは正反対でした。動画機能や早すぎる段階でのアルゴリズムフィードも見送りました。どちらも、コアループが機能すると証明する前に、「レシピを読み取れるオブジェクトにする」という課題をより難しくしてしまうからです。",
    'portfolio.p1.feedback': "まだクローズドベータ中なので、今のところの一番明確な手がかりは大規模な調査ではなく実際の利用状況です。もっとも注視している数字は継続率で、それを改善するための現在の実験がゲーミフィケーションです。もしこれを読んでいてベータテスターに興味があれば、ぜひ声をかけてください!",
    'portfolio.p1.impact': "現在のベータユーザーは約34人、もっとも重視している指標である初週継続率は約21%です。",
...
'portfolio.p2.title': "レビュー感情分析プラットフォーム - インタラクティブマップ",
    ...
    'portfolio.p2.problem': "評判の変化は確かに存在しますが、研究するのは難しいものです。何千もの個々の意見に散らばっていて、時間とともに動き続けており、そのどれもレビューを一件ずつ読むだけでは見えてきません。",
    'portfolio.p2.videoCaption': "キャプションを追加",
    'portfolio.p2.targetUser': "学校の社会科学チームの研究者たち。個々のレビューを一件ずつ読むのではなく、世論の時系列パターンを把握する必要がありました。",
    'portfolio.p2.competitors': "既存のレビューダッシュボード(Google自体の平均評価や、TripAdvisorのような集計サービス)は静的なスコアしか示さず、時間や地域による変化はわかりません。この研究以前に研究者たちが取っていた手作業の回避策は、レビューを一件ずつ読んで感情を手で集計することでした。正確ではありますが、何千件ものレビューからパターンを見つけるにはあまりに遅すぎます。",
    'portfolio.p2.insight': "最初のバージョンは連続的なヒートマップで表示していて、一見説得力があるように見えましたが、実際はそうではありませんでした。ヒートサーフェスは地理空間上の補間を前提としますが、実際のデータは離散的で、サンプリングの密度も場所によって偏ったレビュー点の集まりです。中心部の密集したブロックと、まばらな住宅地では統計的な重みがまったく異なるのに、グラデーションは静かに「ここにどれだけデータがあるか」を「ここの感情がどれだけ強いか」であるかのように表示していたのです。解決策は個別のマーカーに切り替えることでした。密度と強度がひとつの誤解を招く色に混同されないよう、それぞれをサイズと色で分けて表現しました。",
    'portfolio.p2.solution': "Googleレビューをスクレイピングし、より粗い星評価だけでなく、テキスト自体にRoBERTaによる感情分析をかけ、その結果を年スライダー付きのLeafletマップ上に表示しました。時間の経過による変化が、まず最初に目に入るようにするためです。より難しかった設計判断は、データを滑らかにしたい誘惑に抵抗することでした。点と点の間の色を補間する代わりに、個々にプロットしました。滑らかにする方が見た目はきれいですが、これほど少ないデータ点数では、静かに嘘をつくことになるからです。あえて作らなかったもの:複数都市への対応や一般公開版です。このプロジェクトはひとつの地区、ひとつの研究課題に意図的に絞り込んでおり、一般化すればこのプロジェクトの本質だった年単位の精度を犠牲にすることになったはずです。",
...
'portfolio.p3.title': "音声操作型VR建築スタジオ",
    ...
    'portfolio.p3.problem': "3Dの建物デザインを試行錯誤するのは時間がかかり、本当のボトルネックはアイデアそのものではありません。特にゲーマーではない建築家にとって、それを素早く3Dの形にするまでの摩擦こそが問題でした。",
    ...
    'portfolio.p3.targetUser': "初期段階の建物コンセプトを試作する建築家や建築学生 - 技術系の専門家からデザイン志向のアマチュアまで幅広く対象としつつ、ゲーム機の操作に慣れている人だけを想定してはいません。",
    'portfolio.p3.competitors': "RhinoやSketchUpのような従来型ツールは精度が高い一方、初期段階のコンセプト作業を素早く試行錯誤するには向いていません。手作業での回避策は今も発泡スチロールや厚紙のマッシングモデルです。Gravity Sketchのような既存のVRデザインツールは、空間内で3D作業をする部分は解決していますが、依然としてハンドコントローラーに依存しており、ゲーマーではない建築家にとって取り除きたかった障壁をそのまま再導入してしまいます。",
    'portfolio.p3.insight': "最初のバージョンは、壁の種類を選ぶ、寸法を調整する、素材を変えるといった一般的な操作をまだメニューに頼っていました。典型的なVRアプリのパターンです。しかしこれはVRという媒体そのものと相性が悪いものでした。建築家たちは空間の中でリアルタイムに考え、試行錯誤しようとしているのに、メニューに潜るたびにその流れが断ち切られ、本来3Dであるはずのツールの中で2D的な操作モデルに引き戻されてしまっていたのです。気づきはバグを見つけたことではなく、問題そのものが「操作にかかるコスト」にあると理解した瞬間でした。どのメニュー項目がわかりにくいかではなく、メニューが存在すること自体が問題だったのです。Geminiを介した音声操作によって、階層をたどる代わりに、変更したい内容を普通の言葉で説明できるようになりました。",
    'portfolio.p3.solution': "欲しいものを言葉で説明すると、Gemini APIがその自由な自然言語を構造化された3Dアクションへと変換するVRスタジオを構築しました。システムが何を理解したかをユーザーが確認してから確定できるよう、キーワード要約も画面に表示しています。これは、正確な入力ではなく曖昧さと意図のためにデザインすることを意味していました。実際の作業の多くは、人が実際に使う言い回しに対してシステムを柔軟にすることに費やされ、それはヘッドセットへの実装前に9人のユーザーと7回のデザイン改善を重ねることでしか見つけられませんでした。あえて作らなかったものも同じくらい重要です。複数人での共同作業、そして本当の意味での自由な音声入力です。発話はGeminiを通して処理しましたが、それでも意図の範囲はある程度限定し、任意の発話をすべて受け付けるようにはしませんでした。本当に重要なコマンドの信頼性を優先して、コマンドの幅を犠牲にしたということです。物理演算や永続化、エクスポート機能も実装していません。このツールは本格的な制作パイプラインではなく、初期段階の素早い試行錯誤に絞り込んだものです。",
    'portfolio.p3.feedback': "もっとも明確な反応は、言葉ではなく行動から得られました。当たり前だと思っていたコマンドで誰かがつまずく様子を見ることは、後から聞くどんな言葉よりも役に立ちました。7回の改善のほとんどはそこから生まれています。「当たり前」のコマンドが最初から本当に当たり前だったことは、ほとんどありませんでした。",
...
'portfolio.p4.title': "視覚障がい者のための画像処理とメカトロニクス",
    ...
    'portfolio.p4.problem': "視覚障がいのある観客はスタジアムの音や熱気は感じられますが、試合を本当に理解するために必要なたった一つの情報、つまり「今、ボールがどこにあるか」を見失ってしまいます。",
    ...
    'portfolio.p4.targetUser': "試合を生で観戦する視覚障がいのある観客。プレーが実際に起きている最中にそれを追えるだけの速さで、位置情報を必要としています。後からではなく。",
    'portfolio.p4.competitors': "音声解説はすでにほとんどのスタジアムに存在しますが、それは文章で語る人間による解説であり、継続的な位置データではありません。本質的な遅れがあり、ピッチ上のどこにボールがあるかを秒単位で正確に伝えることもできません。今日よく使われる手作業の回避策は、観客の隣に座った目の見える付き添いが、その場でボールの位置を小声で伝え続けることです。",
    'portfolio.p4.insight': "最初のバージョンでは、より重く高解像度な検出モデルを使い、精度を優先して遅延は二の次として扱っていました。実際のエッジ/Arduino側のパイプラインで動かした途端、それは破綻しました。「ボールは左側にあります」という空間的に正確な情報でも、2秒遅れて届けば、単に役に立たないだけでなく、積極的に誤解を招くものになります。すでに終わったプレーを感じることになってしまうからです。気づきは、精度は意味を持つタイミングで届いて初めて価値を持つという点にありました。そこでモデルの精度と解像度を、フレームレートと遅延のために手放すことにし、より重い検出器の代わりに、より軽量なパイプライン上でYOLOv9を使う方向に進みました。",
    'portfolio.p4.solution': "遅延を最も重要な制約とし、その代わりとして許容できる誤差を受け入れました:推論時間180ミリ秒、位置精度は約6メートルです。それを12cmの感覚ボード上で指に感じられる触覚バンドへと変換しています。この約6メートルの誤差は、完璧な座標がなくても「ボールは左サイド、ペナルティエリア付近」と伝えるには十分な精度です。同じ考え方がハードウェアの設計も導きました。「明らかに良さそう」な角度を推測するのではなく、実際のカバー範囲と精度の最適なバランスを見つけるために60通りのカメラ配置を検証しました。あえて作らなかったもの:全選手のトラッキングシステムです。すべての選手を追跡すれば、視覚障がいのある観客が観客の歓声や解説からすでにある程度得られている情報のために、遅延と複雑さを増やすことになったはずです。本当に欠けていた唯一の信号は、ボールの位置でした。",
    'portfolio.p4.impact': "フランスの理工系学生向け研究コンテストTIPEで全国上位3%にランクインし、20/20の評価を得ました。選手や観客など、スポーツを取り巻くさまざまな立場の人たちを想像したことが、このプロジェクトの出発点でした。「技術的にすごいことは何か」ではなく「取り残されているのは誰か」から始めること。それが今も変わらない、私の課題の選び方です。",
  ...
  de: {
    ...
    'portfolio.showLess': "Überlegung ausblenden ↑",
    ...
    'portfolio.p1.problem': "Ein guter Foto-Look lässt sich leicht bewundern und kaum reproduzieren - die Technik dahinter bleibt in der App oder im Kopf der Person, die bearbeitet hat, und verschwindet in dem Moment, in dem das Foto gepostet wird.",
    'portfolio.p1.targetUser': "Hobby-Fotobearbeiter:innen, die Bearbeitung bereits als Handwerk begreifen - die Art Mensch, die einen Look, den sie liebt, als Screenshot speichert und dann aufgibt, ihn nachzubilden.",
    'portfolio.p1.competitors': "VSCO und Lightroom lassen einen Presets speichern und teilen, aber als eigenständige Dateien, losgelöst von dem Foto, das sie inspiriert hat - man kann keinen Look sehen, den man liebt, und mit einem Tap dessen genaues Rezept bekommen. Außerhalb von Apps besteht der eigentliche Workaround darin, einen Look als Screenshot zu speichern und in den Kommentaren zu fragen „welcher Filter ist das?“ - und dann nach Augenmaß nachzubauen.",
    'portfolio.p1.insight': "Die meisten Foto-Apps teilen das Ergebnis: das fertige Bild. Aber das eigentlich Wertvolle, das Übertragbare, ist nicht das Ergebnis - es ist die Transformation. Mein erster Instinkt tendierte zu einem Preset-Marktplatz - statische Looks, die man anwendet, nicht weit weg von einem Filterpaket. Das scheiterte schon in frühen Skizzen: Ein Filter ist statisch, er passt sich nicht an das jeweilige Foto an. Der Durchbruch kam, als mir die eigentliche Form des Problems klar wurde: Jemand fragt „wie hast du diesen Look hinbekommen?“, und die ehrliche Antwort ist ein Absatz voller Einstellungen, die niemand von Hand nachbauen wird. Genau diese Lücke - zwischen einen Look bewundern und ihn tatsächlich reproduzieren können - ist das Produkt.",
    'portfolio.p1.solution': "Ich habe das Rezept zum zentralen Objekt der App gemacht: Feed, Profil und Editor mussten Bearbeitungen lesbar und remixbar machen, nicht nur fertige Bilder zeigen. Der Ansatz mit gesperrten Ebenen (inspiriert von VSCO) war ebenfalls eine bewusste Einschränkung: Ich habe mich entschieden, Nutzer:innen die Bearbeitung anderer nicht bis auf die Rohwerte zerlegen zu lassen - maximale Remixbarkeit wurde zugunsten der Namensnennung der ursprünglichen Ersteller:in geopfert. Und ich habe mich entschieden, Bearbeitungen mit Skia-Shadern direkt auf dem Gerät laufen zu lassen statt über ein einfacheres serverseitiges Rendering, weil eine soziale Schleife stirbt, sobald sich Anpassen wie Warten anfühlt. Der Kompromiss, über den ich am ehrlichsten sprechen kann: Ich habe eine vollständige Gamification-Ökonomie entworfen - Punkte fürs Posten, Punkte fürs Gespeichertwerden, einen Shop, um sie auszugeben - und mich dann entschieden, sie nicht komplett auszurollen. Frühe Modellierungen zeigten, dass sie eine Reich-werden-noch-reicher-Dynamik erzeugen würde: Frühe Nutzer:innen, die speichern, bauen einen Vorsprung auf, und Neueinsteiger:innen kommen nicht rein - das Gegenteil dessen, was ein Entdeckungs-Feed leisten sollte. Ich habe außerdem Video und einen frühen algorithmischen Feed zurückgestellt, da beides das Problem „Rezept als lesbares Objekt“ erschwert hätte, bevor überhaupt bewiesen war, dass der Kernkreislauf funktioniert.",
    'portfolio.p1.feedback': "Noch in geschlossener Beta, daher kommt das klarste Signal bisher aus der tatsächlichen Nutzung, nicht aus einer breiten Umfrage. Nutzerbindung ist die Zahl, die ich am genauesten beobachte, und Gamification ist das aktuelle Experiment, um sie zu verbessern. Wenn Sie das hier lesen und Beta-Tester:in werden möchten, melden Sie sich gerne!",
    'portfolio.p1.impact': "Bisher rund 34 Beta-Nutzer:innen, mit einer Woche-1-Retention von etwa 21% als der Kennzahl, auf die ich am meisten hinarbeite.",
    ...
    'portfolio.p2.problem': "Ein Wandel des Rufs ist real, aber schwer zu untersuchen: Er ist über Tausende einzelne Meinungen verstreut, verändert sich über die Zeit, und nichts davon ist sichtbar, wenn man Bewertungen nur einzeln liest.",
    'portfolio.p2.videoCaption': "Bildunterschrift hier einfügen",
    'portfolio.p2.targetUser': "Die Forschenden des Sozialwissenschafts-Teams der Hochschule, die Muster in der öffentlichen Meinung über die Zeit erkennen mussten - nicht nur einzelne Bewertungen nacheinander lesen.",
    'portfolio.p2.competitors': "Fertige Bewertungs-Dashboards (Googles eigener Bewertungsdurchschnitt, TripAdvisor-artige Aggregatoren) zeigen nur einen statischen Wert, nicht wie er sich über Zeit oder Raum verändert. Der manuelle Workaround der Forschenden vor diesem Projekt war, Bewertungen einzeln zu lesen und das Sentiment von Hand zu zählen - genau, aber viel zu langsam, um ein Muster über Tausende Bewertungen hinweg zu erkennen.",
    'portfolio.p2.insight': "Die erste Version zeigte eine durchgehende Heatmap, die autoritativ wirkte, es aber nicht war. Eine Wärmefläche unterstellt Interpolation über den Raum hinweg, aber die zugrundeliegenden Daten sind tatsächlich diskrete, ungleichmäßig verteilte Bewertungspunkte - ein dichter Innenstadtblock und ein dünn besiedeltes Wohnviertel haben nicht dasselbe statistische Gewicht, also codierte der Farbverlauf still „wie viele Daten gibt es hier“ so, als wäre es „wie stark ist das Sentiment hier“. Die Lösung war der Wechsel zu einzelnen Markern, deren Größe und Farbe Dichte und Intensität getrennt hielten, statt sie in einer einzigen irreführenden Farbe zu vermischen.",
    'portfolio.p2.solution': "Wir haben Google-Bewertungen gescrapt, eine RoBERTa-Sentiment-Analyse auf den eigentlichen Text angewendet (nicht nur auf Sternebewertungen, die gröber sind), und das Ergebnis auf einer Leaflet-Karte mit Jahres-Schieberegler dargestellt, damit Veränderung über die Zeit das Erste ist, was man sieht. Die schwierigere Design-Entscheidung war, dem Drang zu widerstehen, die Daten zu glätten: Statt Farben zwischen Punkten zu interpolieren, habe ich sie einzeln dargestellt, denn Glättung sieht hübscher aus, verfälscht aber leise bei so wenigen Datenpunkten pro Ort. Was wir bewusst nicht gebaut haben: Unterstützung für mehrere Städte oder eine öffentliche Version - dieses Projekt war bewusst eng auf eine Forschungsfrage für ein Viertel zugeschnitten, und es zu verallgemeinern hätte bedeutet, die Jahr-für-Jahr-Präzision zu opfern, die den eigentlichen Sinn der Sache ausmachte.",
    ...
    'portfolio.p3.problem': "An einem 3D-Gebäudeentwurf zu iterieren ist langsam, und der eigentliche Engpass sind nicht die Ideen - es ist die Reibung, sie schnell in eine nutzbare 3D-Form zu bringen, besonders für Architekt*innen, die keine Gamer sind.",
    ...
    'portfolio.p3.targetUser': "Architekt*innen und Architekturstudierende, die frühe Gebäudekonzepte prototypisieren - technische Profis ebenso wie designorientierte Amateur*innen, aber nicht Menschen, die mit spielähnlichen Controllern vertraut sind.",
    'portfolio.p3.competitors': "Klassische Werkzeuge (Rhino, SketchUp) sind präzise, aber langsam zu iterieren für frühe Konzeptarbeit, und der manuelle Workaround ist nach wie vor ein physisches Massenmodell aus Schaumstoff oder Karton. Bestehende VR-Design-Tools wie Gravity Sketch lösen den Teil „in 3D im Raum arbeiten“, setzen aber weiterhin auf Handcontroller - was genau die Hürde wieder einführt, die wir für nicht-gamende Architekt*innen beseitigen wollten.",
    'portfolio.p3.insight': "Die erste Version setzte für gängige Aktionen - einen Wandtyp wählen, eine Abmessung anpassen, ein Material austauschen - noch auf Menüs, das übliche VR-App-Muster. Das kämpfte gegen das Medium an: Architekt:innen versuchten, in Echtzeit räumlich zu denken und zu iterieren, und jeder Ausflug ins Menü unterbrach diesen Fluss und zog sie zurück in ein 2D-Interaktionsmodell innerhalb eines eigentlich 3D-Tools. Der Durchbruch war kein gefundener Bug - es war die Erkenntnis, dass die Interaktionskosten selbst das Problem waren, nicht welcher Menüpunkt falsch beschriftet war. Sprache, geleitet über Gemini, ließ Menschen eine Änderung in normalen Worten beschreiben, statt sich durch eine Hierarchie zu klicken.",
    'portfolio.p3.solution': "Wir haben ein VR-Studio gebaut, in dem man beschreibt, was man will, und die Gemini API diese lockere natürliche Sprache in strukturierte 3D-Aktionen übersetzt - mit einer Stichwort-Zusammenfassung, die den Nutzer*innen zeigt, was das System verstanden hat, bevor sie bestätigen. Das bedeutete, für Mehrdeutigkeit und Absicht zu gestalten statt für präzise Eingaben - der Großteil der eigentlichen Arbeit bestand darin, das System tolerant dafür zu machen, wie Menschen Dinge wirklich formulieren, was wir nur durch 7 Design-Iterationen mit 9 Nutzer*innen vor dem Einsatz im Headset herausfanden. Was wir bewusst nicht gebaut haben, war genauso wichtig: Mehrbenutzer-Zusammenarbeit, und wirklich freie Spracheingabe - wir haben Sprache über Gemini geleitet, aber den Intentionsraum trotzdem eingegrenzt, statt beliebige Befehle zu akzeptieren, und damit Bandbreite gegen Zuverlässigkeit bei den Befehlen getauscht, die wirklich zählten. Auch keine Physik, Persistenz oder Export-Pipeline: Das Tool war auf schnelle frühe Iteration zugeschnitten, nicht auf eine vollständige Produktions-Pipeline.",
    'portfolio.p3.feedback': "Das eindeutigste Feedback war verhaltensbasiert, nicht verbal: zuzusehen, wie jemand an einem Befehl scheitert, den wir für offensichtlich hielten, war nützlicher als alles, was danach gesagt wurde. Das trieb die meisten der 7 Iterationen an - der „offensichtliche“ Befehl war es beim ersten Versuch selten wirklich.",
    ...
    'portfolio.p4.problem': "Sehbehinderte Zuschauer*innen bekommen Klang und Atmosphäre eines Stadions mit, verlieren aber genau die eine Information, die das Spiel eigentlich erklärt: wo sich der Ball gerade befindet.",
    ...
    'portfolio.p4.targetUser': "Blinde und sehbehinderte Fußballzuschauer*innen bei einem Live-Spiel, die Positionsinformationen schnell genug brauchen, um eine Spielszene mitzuverfolgen, während sie passiert - nicht danach.",
    'portfolio.p4.competitors': "Audiokommentar gibt es in den meisten Stadien bereits, aber das ist ein Mensch, der in Sätzen erzählt, keine kontinuierlichen Positionsdaten - es gibt eine inhärente Verzögerung, und er kann nicht vermitteln, wo genau auf dem Feld sich der Ball gerade befindet, Sekunde für Sekunde. Der übliche manuelle Workaround heute ist eine sehende Begleitperson, die neben der zuschauenden Person sitzt und ihr leise die Ballposition in Echtzeit ansagt.",
    'portfolio.p4.insight': "Mein erster Durchlauf nutzte ein schwereres, höher auflösendes Erkennungsmodell und jagte zunächst der Genauigkeit hinterher, Latenz war zweitrangig. Das brach zusammen, sobald es auf dem tatsächlichen Edge-/Arduino-Pfad laufen musste: Ein räumlich genaues „der Ball ist links von Ihnen“, das zwei Sekunden zu spät ankommt, ist nicht nur nutzlos, sondern aktiv irreführend, weil man dann eine bereits vergangene Aktion spüren würde. Der Durchbruch war die Erkenntnis, dass Genauigkeit nur zählt, wenn sie rechtzeitig ankommt, um etwas zu bedeuten - also habe ich Modellgenauigkeit und Auflösung gegen Bildrate und Latenz eingetauscht, was mich zu YOLOv9 auf einer leichteren Pipeline statt eines schwereren Detektors geführt hat.",
    'portfolio.p4.solution': "Ich habe Latenz zur wichtigsten Einschränkung gemacht und im Gegenzug eine vertretbare Fehlermarge akzeptiert: 180 ms Inferenzzeit und etwa 6 Meter Lokalisierungsgenauigkeit, umgewandelt in eine haptische Bande, die die Nutzer*innen unter den Fingern auf einem 12-cm-Sensorboard spüren. Diese ~6 m Fehler reichen aus, um „Ball am linken Flügel, nahe dem Strafraum“ zu vermitteln, ohne perfekte Koordinaten zu brauchen. Dieselbe Logik hat auch die Hardware geprägt: Ich habe 60 Kamerapositionen getestet, um den tatsächlichen Sweet Spot zwischen Abdeckung und Genauigkeit zu finden, statt einen „offensichtlich guten“ Winkel zu erraten. Was ich bewusst nicht gebaut habe: ein System zur Verfolgung aller Spieler*innen - das hätte Latenz und Komplexität für Informationen hinzugefügt, die sich blinde Zuschauer*innen teilweise bereits aus Publikumslärm und Kommentar erschließen können; die Ballposition war das eine Signal, das wirklich fehlte.",
    'portfolio.p4.impact': "Landesweit unter den besten 3% beim TIPE-Forschungswettbewerb für Ingenieurstudierende in Frankreich platziert, mit 20/20 bewertet. An all die verschiedenen Menschen rund um den Sport zu denken - Spieler*innen, Zuschauer*innen - hat mich zu diesem Projekt geführt. Und von „Wer wird ausgeschlossen?“ statt von „Was ist technisch beeindruckend?“ auszugehen, ist bis heute meine Art, Probleme auszuwählen.",
